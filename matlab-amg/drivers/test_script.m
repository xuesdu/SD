...
% 
%load('../data/EX4.mat')
%load('../data/airfoil1_dual.mat')
% 
% %load('../data/J12800_2D_p1.mat')
%A = -Problem.A;
% 
% 
% %A = J12800; 
% 
% %load('../data/J12800_2D_p2.mat')
% %A = J12800; 
% 
% %load('../data/SGDG-6.mat')
% %A = L_SG;
% 
% %A = Avg;
% 
...
%  %Ld_d = rand(N,1);
%  Ld_d = ones(N,1);
%  Ld_d_tilde = mean(Ld_d);
%  b = Ld_b - (Ld_d.*u) * (ones(1,N)*Ld_b)/Ld_d_tilde;
%---------------------
% AMG parameters
%---------------------
...
amgParam.coarse_it = 1; % Number of iterations on coarse levels. Used when amgParam.cycle_type = 'nV' or 'K'
...
amgParam.tol = 1e-7;    % when AMG is used as standalone solver, tolerance for the reletive residual
amgParam.threshold = 0.4;    % resetup criterion. Upper bound of avg. convergence rate
...
iterParam.tol = 1e-7;  % Tolerance for relative residual 
...
switch iterParam.solver_type
    ...
    case 'AMG_Adapt' % use adaptive AMG solver
        ...
        % use hierarchies from solving Ax=0 and use it to solve Ax=b
        %save_amgData = AMG_Setup_Adapt0(L, x, amgParam);
        %x = AMG_Solve_Adaptb(L, x, b, amgParam, save_amgData);
        
        % solve Ax=b to get exact_x and use it as resetup criterion
%         x_approx_sol = AMG_Setup_Adaptb(L, b, x, amgParam);
%         x = AMG_Solve_Adapt(L, b, x, amgParam, x_approx_sol);

    otherwise, % use preconditioned Krylov methods
        try
            x = Krylov_Solve(L, b, x, iterParam, amgParam, amgData);
        catch
            x = Krylov_Solve(L, b, x, iterParam, amgParam, []);  % when AMG is not called
        end
        x = x - x' * ones(N,1) / N;
