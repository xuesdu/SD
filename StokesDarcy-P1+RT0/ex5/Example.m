...
% to verify the Strong impose the mass conservation, we need the P1 part

us1 = (exp(x)-exp(1))*cos(pi*y) + 1; % only one difference from ex1
us2 = -exp(x)*sin(pi*y)/pi;
ps = 2*exp(x)*cos(pi*y);

ud1 = -(exp(x)-exp(1))*cos(pi*y) + 1;
