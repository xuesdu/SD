function b = assemble_vector(Omega,fun,para,dof,Tb_test,Gauss_weights_ref_2D,Gauss_nodes_ref_2D,number_of_local_basis_test,...
...
if Omega == 's'
    for n = 1:number_of_elements/2 % 遍历所有单元
        ...
        for beta = 1:number_of_local_basis_test
            int_value = Gauss_quad_test(fun,para,Gauss_weights_2D,Gauss_nodes_2D,vertices,basis_type_test,beta,basis_vector_test,basis_der_x_test,basis_der_y_test);
            i = Tb_test(beta,n);
            b(i) = b(i) + int_value;
        end
    end
