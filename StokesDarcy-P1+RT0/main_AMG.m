...
% Stokes velocity us and pressure ps are approximation by P1+RT0-P0;
% Darcy velocity ud and pressure pd are approximation by RT0-P0;
...
gamma_tilde = 1;
Gpn = 9;  
e_us = [];  e_ps = [];  e_ud = [];  e_pd = []; cond_number = 0; cond_number_eff = 0;  

BC = 'I';  % different boundary conditions
temp = 0;
for ch = 0:3    % h = 1/2^(ch+1)
    temp = temp+1;
    prog = select(1,ch);
    if prog.end == 1
        return
    ...
    [P,T,E,Inter] = generate_P_T(2*Nx,Ny,201);  
    ...
    % Assemble
    [A0, b0, A, b, Proj, solution_g] = assemble_matrix_vector_XH(para,BC,Nx,Ny,dof_ul,dof_uR,dof_ps,dof_ud,dof_pd,...
    ...
    % fprintf("cond_number = %.2e, cond_number_eff = %.2f\n",cond_number,cond_number_eff);
    ...
    amgParam.Schwarz_level = 1;  % 1:use block smoother; 
