function A = assemble_matrix_Stokes_RT0(Omega,fun,dof_trial,dof_test,Tb_test,Tb_trial,Gauss_weights_ref_2D,Gauss_nodes_ref_2D,number_of_local_basis_trial,number_of_local_basis_test,...
...
%             for beta = 1:number_of_local_basis_test
                int_value = Gauss_quad_trial_test(fun,Gauss_weights_2D,Gauss_nodes_2D,vertices,...
                    ...
                    basis_type_test,alpha,basis_vector_test,basis_der_x_test,basis_der_y_test);
%                 i = Tb_test(beta,n);
                j = Tb_trial(alpha,n);
                A(j,j) = A(j,j) + int_value;
%             end
        end
    end
elseif Omega == 'd'
