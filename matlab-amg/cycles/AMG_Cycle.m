...
if (level == max_level) 
    ...
    elseif level <= Schwarz_level
        x = forward_schwarz(amgData(level).A, b, x, n_presmooth, amgData(level).schwarzData);
        %if (isfield(amgParam, "Schwarz_blocks"))
    ...
    switch cycle_type
        ...
        case 'Hu' %AMLI-cycle using Hu polynomial 
            if (level == max_level-1)
                e_c = AMG_Cycle(amgData, r_c, e_c, level+1, amgParam);
            else
                alpha = 0.2; e_0 = e_c;
                switch amgParam.coarse_it
                    ...
                    case 2
                        L = (1+alpha)^2/(4*alpha);
                    otherwise
                        %L = (1+alpha)^2/(4*alpha); % needs modified
                        L = 1 + ((amgParam.coarse_it-1)^(amgParam.coarse_it-1)/(amgParam.coarse_it^amgParam.coarse_it))*((1-alpha)^(amgParam.coarse_it)/alpha);
                end
                w = 1/(alpha*L);
                %e_c = AMG_Cycle(amgData, w*r_c, e_c, level+1, amgParam);
                e_c = e_c + w*AMG_Cycle(amgData, r_c-amgData(level+1).A*e_c, e_0, level+1, amgParam);
                for i = 2:amgParam.coarse_it
                    w = 1/L;
                    %e_c = AMG_Cycle(amgData, w*r_c, e_c, level+1, amgParam);
...
%                    @(r)AMG_Cycle(amgData, r, e_0, level+1, amgParam));
    ...
    elseif level <= Schwarz_level
        x = backward_schwarz(amgData(level).A, b, x, n_postsmooth, amgData(level).schwarzData);
        %if (isfield(amgParam, "Schwarz_blocks"))
        %    x = backward_gs(amgData(level).A, b, x, amgData(level).DL, n_presmooth);
        %end
    else
        switch smooth_type
            case 'Rd',
