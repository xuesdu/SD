function [bn,be] = generate_boundary_nodes_edges_Omegaf(Nx_basis,Ny_basis,Nx,Ny,BC)
...
% 上边界的界面点
% k = (Nx_basis-1)+Ny_basis+1;      % 第 k 个边界点
% id = (Nx_basis+1)*(Ny_basis+1);   % 全局编号
% bn(2,k) = id;
% bn(1,k) = -1;

% 左边界
for j = 1:Ny_basis+1
    k = 2*Nx_basis-1+Ny_basis+j;
    id = Ny_basis+1-(j-1);
    bn(2,k) = id;
    switch BC
        case 'I' 
...
% be(6,k): the type of boundary condition
