% 涉及两个单元
function int_value = Gauss_quad_line_trial_test_Omegaf_Omegap(fun,end_point_1,end_point_2,Gauss_weights_ref_1D,Gauss_nodes_ref_1D,vertices_trial,vertices_test,...
...
if end_point_1(2) == end_point_2(2)  % 水平边界
    ...
    for i = 1:Gpn
        ...
        int_value = int_value + Gauss_weights_1D(i)*fun(x,y)*...
            FE_basis_fun_local(x,y,vertices_trial,edge_index,basis_type_trial,basis_index_trial,basis_vector_trial,basis_der_x_trial,basis_der_y_trial)*...
            FE_basis_fun_local(x,y,vertices_test,edge_index,basis_type_test,basis_index_test,basis_vector_test,basis_der_x_test,basis_der_y_test)'*normal_vector;
    end
elseif end_point_1(1) == end_point_2(1)  % 竖直边
    ...
    for i = 1:Gpn
        ...
        int_value = int_value + Gauss_weights_1D(i)*fun(x,y)*...
            FE_basis_fun_local(x,y,vertices_trial,basis_type_trial,basis_index_trial,basis_vector_trial,basis_der_x_trial,basis_der_y_trial)*...
            FE_basis_fun_local(x,y,vertices_test,basis_type_test,basis_index_test,basis_vector_test,basis_der_x_test,basis_der_y_test);
    end
else  % 斜边
    lb = min(end_point_1(1),end_point_2(1));
    ub = max(end_point_1(1),end_point_2(1));
    normal_vector = [sqrt(2)/2;sqrt(2)/2];
    [Gauss_weights_1D,Gauss_nodes_1D] = Gauss_local_1D(lb,ub,Gauss_weights_ref_1D,Gauss_nodes_ref_1D);
